...
namespace FighterXD.Main
{
    class Terrain : Object
    {
        ...
        protected override void AddInfoToCopy(Object copy)
        {
            ...
            ((Terrain)copy).width = width;
            ((Terrain)copy).height = height;
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ((Terrain)copy).t[x, y] = (TerrainObject)t[x, y].CreateCopy();
                }
            }
        }
    ...
    class TerrainObject : ExplodableObject
    {
        ...
        {
            base.AddInfoToCopy(copy);
            ((TerrainObject)copy).neighbors = neighbors;
            ((TerrainObject)copy).x = x;
            ((TerrainObject)copy).y = y;
        }
    }
}
